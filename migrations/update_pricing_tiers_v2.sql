...
CREATE INDEX IF NOT EXISTS idx_free_tier_stocks_category ON free_tier_stocks(category);
CREATE INDEX IF NOT EXISTS idx_free_tier_stocks_symbol ON free_tier_stocks(symbol);

-- Populate with initial sample stocks (top dividend aristocrats & kings)
INSERT INTO free_tier_stocks (symbol, name, category) VALUES
...
BEGIN
    ...
    IF v_coverage_type = 'sample' THEN
        RETURN EXISTS (
            SELECT 1 FROM free_tier_stocks WHERE symbol = p_symbol
        );
    END IF;
...
FROM divv_api_keys ak
LEFT JOIN tier_limits tl ON ak.tier = tl.tier;
...
COMMENT ON TABLE tier_limits IS 'Configuration table storing rate limits and features for each pricing tier';
COMMENT ON TABLE rate_limit_tracking IS 'Tracks API usage for rate limiting in distributed systems';
COMMENT ON TABLE free_tier_stocks IS 'Curated list of stocks accessible to free tier users';
...
BEGIN
    ...
    RAISE NOTICE '   Free:         10,000/month,  10/min (burst: 20)';
